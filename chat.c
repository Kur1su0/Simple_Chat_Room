 ...
 *
 * Known bugs: thers is bug when input text on two terminals at the same time.
 *
 *
...
const char *welcome = "Welcome to chat room";
const char *remote_name = "apolloXX";


struct TEXT{
    char text[MAXBUFLEN+1];
};
...
//
//
int chat_talker(int sockfd, struct TEXT *ptr_s)
...
if (ch=='\n'){
    ...
    flag = 1;
        ...
        flag =1;
                  ...
                  buf1[i_s++] =  ch;         // but what about buffer overflow!
                  }
               else   if (ch==KEY_BACKSPACE){
                            if(buf1[i_s-1] !='\0' ){
                            i_s--;
...
if(flag ==1){
    ...
    if (strcmp(buf1,"quit")==0 ) quit_flag =1;

    if ((numbytes = send(sockfd, buf1, strlen(buf1), 0)) == -1) {
        perror("chat: send");
        exit(1);}
strcpy(ptr_s[0].text,buf1);

 for (j = 0  ;j<=count_s; j++){
      //  move(LINES/2-1 -j,0);// Move Cursor to Location
      move(LINES -2 -j,0);
        clrtoeol();
        addstr(ptr_s[j].text);
    }
if (count_s < screen_middle-3)  count_s++;


//if (count_s < screen_middle)  count_s++;
for(j=count_s; j>0;j--){
     strcpy(ptr_s[j].text,ptr_s[j-1].text);
}
...
//
void chat_listener(int sockfd, struct TEXT  *ptr_r )
{
    char buf[MAXBUFLEN];
    int numbytes;
...
strcpy(ptr_r[0].text,buf);

//refresh();

 for (j = 0  ;j<=count_r; j++){
      //  move(LINES -2 -j,0);// Move Cursor to Location
        move(LINES/2-1 -j,0);// Mo
        clrtoeol();
        addstr(ptr_r[j].text);
    }

//if (count_r < LINES/2-3)  count_r++;
if (count_r < LINES/2-1)  count_r++;


for(j=count_r; j>0;j--){

     strcpy(ptr_r[j].text,ptr_r[j-1].text);
}

  move(y,x);  // Move Cursor to Lower Left
        refresh();
...
int main(int argc, char *argv[])
{
 ...
 fd_set master;
     ...
     int screen_middle; char *screen_divider, *addr;
     struct TEXT *ptr_s;
     struct TEXT *ptr_r;

     //malloc here------
  //   ptr  = (struct TEXT*) malloc( (1000)* sizeof(struct TEXT) );
     //----------------
     ...
     ptr_s = (struct TEXT*) malloc( (screen_middle-1)* sizeof(struct TEXT) );
     ptr_r = (struct TEXT*) malloc( (LINES/2)* sizeof(struct TEXT) );
    ...
    while(quit_flag == 0) {
      // specify who to monitor on this trial.  We want both
        ...
        if (FD_ISSET(0, &read_fds)) {
            quit_flag = chat_talker(sockfd,ptr_s);      // keyboard input
        }
        if (FD_ISSET(sockfd, &read_fds)) {
            chat_listener(sockfd,ptr_r);                // remote host sent data
        }
    }
